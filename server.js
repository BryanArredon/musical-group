...
// Middleware para establecer cabeceras de seguridad HTTP (LGPDPPSO y OWASP)
...
const db = new sqlite3.Database(DB_PATH, (err) => {
  if (err) {
    ...
    // Crear tabla de usuarios/integrantes si no existe, incluyendo consentimiento de privacidad
    db.run(
      `
            CREATE TABLE IF NOT EXISTS usuarios (
                ...
                message TEXT,
                rol TEXT NOT NULL,
                privacidad_aceptada INTEGER DEFAULT 0,
                privacidad_aceptada_en TEXT,
      ...
      (err) => {
        if (err) {
          ...
          // Alteración silenciosa por si la tabla ya existía sin las columnas de privacidad
          db.run('ALTER TABLE usuarios ADD COLUMN privacidad_aceptada INTEGER DEFAULT 0', () => {
            // Ignorar si la columna ya existe
          })
          db.run('ALTER TABLE usuarios ADD COLUMN privacidad_aceptada_en TEXT', () => {
            // Ignorar si la columna ya existe
          })
        }
      }
...
app.post('/submit', (req, res) => {
  const { user_name, user_email, user_message, user_rol, privacidad_aceptada } = req.body

  // Validación obligatoria de consentimiento bajo LGPDPPSO
  if (privacidad_aceptada !== 1 && privacidad_aceptada !== true) {
    return res.status(400).json({
      success: false,
      message: 'Es obligatorio aceptar el Aviso de Privacidad de acuerdo con la LGPDPPSO.',
    })
  }
  ...
  const sql = `INSERT INTO usuarios (name, email, message, rol, privacidad_aceptada, privacidad_aceptada_en) VALUES (?, ?, ?, ?, ?, ?)`
  const params = [
    user_name.trim(),
    user_email.trim(),
  ...
  db.run(sql, params, function (err) {
    if (err) {
      console.error('Error al guardar en la base de datos SQLite:', err.message)
    ...
    console.log(`Registro guardado exitosamente. ID: ${this.lastID} con consentimiento LGPDPPSO.`)
    res.status(200).json({
      success: true,
